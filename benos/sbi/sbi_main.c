 ...
 *              当前pmpXcfg字段的address matching mode 只支持 NA4与NAPOT
 * 				一般都是NAPOT，只有在size=4时才会使用NA4
 *
 * reg_idx: pmpXcfg字段的idx，目前只支持0~7  pmpXcfg对应一个pmpaddrX
 * start:Region起始地址
 * size: Region size
 * prot: pmpXcfg字段的属性  RWX
 */
...
void sbi_main(void)
{
	...
	sbi_set_pmp(0, 0x80300000, 0x40000, 0x1);//优先级高
